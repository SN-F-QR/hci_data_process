function sigline(num, xs,lbl,h,yv)
...
if (num == 1)
    plot(mean(xs), y*1.13, '*k', 'LineWidth', 1)% the sig star sign
elseif (num==2)
    plot(mean(xs)-0.04, y*1.13, '*k', 'LineWidth', 1)% the sig star sign
    plot(mean(xs)+0.04, y*1.13, '*k', 'LineWidth', 1)% the sig star sign
elseif (num==3)
    plot(mean(xs)-0.07, y*1.13, '*k', 'LineWidth', 1)% the sig star sign
    plot(mean(xs)+0.07, y*1.13, '*k', 'LineWidth', 1)% the sig star sign
    plot(mean(xs), y*1.13, '*k', 'LineWidth', 1)% the sig star sign
end
if lbl
...
plot([1;1]*xs(1),[y*1.08,y*1.1],'-k', 'LineWidth',2);%left edge drop
plot([1;1]*xs(2),[y*1.08,y*1.1],'-k', 'LineWidth',2);%right edge drop
hold off
